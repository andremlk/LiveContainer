...
static NSString * const LCTVSelectedSlotKey = @"LCTVMVP7HSelectedSlotNextLaunch";
static NSString * const LCTVResultKey = @"LCTVMVP7HResult";
// Keep the MVP7G writable resource location stable across outer-app updates.
...
static NSDictionary *LCTVReadUpdateManifest(void) {
    NSString *path = [NSBundle.mainBundle pathForResource:@"LCTVUpdateManifest" ofType:@"json"];
...
static NSString *LCTVUpdateState(NSDictionary *manifest, NSString *bundleID) {
    if (!bundleID.length) return @"UNKNOWN";
    if ([manifest[@"added"] isKindOfClass:NSArray.class] && [manifest[@"added"] containsObject:bundleID]) return @"ADDED";
    if ([manifest[@"updated"] isKindOfClass:NSArray.class] && [manifest[@"updated"] containsObject:bundleID]) return @"UPDATED";
    if ([manifest[@"preserved"] isKindOfClass:NSArray.class] && [manifest[@"preserved"] containsObject:bundleID]) return @"PRESERVED";
    return @"CURRENT";
}
...
static int LCTVBootImportedGuest(int argc, char *argv[], NSDictionary *slot) {
    if (!slot) {
        LCTVStoreResult(@"MVP7H FAIL: selected imported slot not found");
        return INT_MIN;
    }
    ...
    if (!LCTVEnsureWritableResources(slot, &setupError)) {
        LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ RESOURCE PREP FAIL: %@", displayName, setupError ?: @"unknown"]);
        return INT_MIN;
    }
    ...
    if (!bundlePath.length || !guestPath.length || !bundleID.length || !processName.length) {
        LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ FAIL: update descriptor incomplete", displayName]);
        return INT_MIN;
    }
    NSBundle *resourceBundle = [NSBundle bundleWithPath:bundlePath];
    if (!resourceBundle) {
        LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ FAIL: writable resource NSBundle rejected", displayName]);
        return INT_MIN;
    }
    ...
    if (!LCTVEnsureDirectory(guestHome, &mkdirError)) {
        LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ FAIL guest HOME: %@", displayName, mkdirError ?: @"unknown"]);
        return INT_MIN;
    }
    ...
    LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ START incremental update resources + signed code pre=%@", displayName, preOK ? @"PASS" : @"PARTIAL"]);

    dlerror();
    void *handle = dlopen(guestPath.fileSystemRepresentation, RTLD_NOW | RTLD_GLOBAL);
    if (!handle) {
        const char *raw = dlerror();
        LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ DLOPEN FAIL signed code: %s", displayName, raw ?: "unknown"]);
        return 1;
    }
    const struct mach_header_64 *header = LCTVLoadedHeaderForPath(guestPath);
    if (!header) {
        LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ FAIL: loaded signed image not found", displayName]);
        dlclose(handle);
        return 1;
    }
    uint64_t entryoff = 0;
    ...
    if (!LCTVFindLCMainEntryOffset(header, &entryoff, &mainError)) {
        LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ LC_MAIN FAIL: %@", displayName, mainError ?: @"unknown"]);
        dlclose(handle);
        return 1;
    }
    NSString *postError = nil;
    BOOL postOK = LCTVFinishHostGuestIdentityAfterLoad(header, &postError);
    LCTVStoreResult([NSString stringWithFormat:@"MVP7H %@ PASS update pre=%@ post=%@ frameworks=%lu entryoff=0x%llx",
                     displayName,
                     preOK ? @"PASS" : @"PARTIAL",
                     postOK ? @"PASS" : @"PARTIAL",
...
@interface LCTVMVP7HViewController : UIViewController
@property(nonatomic,strong) UILabel *statusLabel;
@property(nonatomic,strong) NSArray<NSDictionary *> *catalog;
@end

@implementation LCTVMVP7HViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = UIColor.blackColor;
    self.catalog = LCTVBuildImportCatalog();
    NSDictionary *update = LCTVReadUpdateManifest();
    NSDictionary *counts = [update[@"counts"] isKindOfClass:NSDictionary.class] ? update[@"counts"] : @{};

    UILabel *title = [[UILabel alloc] init];
    title.translatesAutoresizingMaskIntoConstraints = NO;
    title.text = @"LiveContainerTV — MVP7H";
    ...
    subtitle.text = @"Incremental IPA Update — preserve existing guests/data + add signed slots";
    subtitle.textColor = UIColor.lightGrayColor;
    subtitle.font = [UIFont systemFontOfSize:21.0];
    subtitle.textAlignment = NSTextAlignmentCenter;

    UILabel *delta = [[UILabel alloc] init];
    delta.translatesAutoresizingMaskIntoConstraints = NO;
    delta.text = [NSString stringWithFormat:@"update plan: preserved %@  •  added %@  •  updated %@  •  removed %@",
                  counts[@"preserved"] ?: @0,
                  counts[@"added"] ?: @0,
                  counts[@"updated"] ?: @0,
                  counts[@"removed"] ?: @0];
    delta.textColor = UIColor.systemGreenColor;
    delta.font = [UIFont monospacedSystemFontOfSize:16.0 weight:UIFontWeightRegular];
    delta.textAlignment = NSTextAlignmentCenter;
    ...
    for (NSDictionary *entry in self.catalog) {
        ...
        button.titleLabel.font = [UIFont boldSystemFontOfSize:26.0];
        ...
        NSString *bundleID = LCTVString(entry[@"bundleID"]) ?: @"unknown";
        details.text = [NSString stringWithFormat:@"%@  •  v%@ (%@)  •  signed main + %@ fw  •  writable code=NO  •  %@  •  %@",
                        bundleID,
                        LCTVString(entry[@"version"]) ?: @"",
                        LCTVString(entry[@"build"]) ?: @"",
                        entry[@"frameworkCount"] ?: @0,
                        storeOK ? @"store=PASS" : @"store=FAIL",
                        LCTVUpdateState(update, bundleID)];
        details.textColor = storeOK ? UIColor.systemGreenColor : UIColor.systemOrangeColor;
        details.font = [UIFont monospacedSystemFontOfSize:15.0 weight:UIFontWeightRegular];
        ...
        [NSLayoutConstraint activateConstraints:@[
            [card.widthAnchor constraintEqualToConstant:1100.0],
            [button.topAnchor constraintEqualToAnchor:card.topAnchor constant:9.0],
            ...
            [button.heightAnchor constraintGreaterThanOrEqualToConstant:56.0],
            [details.topAnchor constraintEqualToAnchor:button.bottomAnchor constant:1.0],
            [details.leadingAnchor constraintEqualToAnchor:card.leadingAnchor constant:20.0],
            [details.trailingAnchor constraintEqualToAnchor:card.trailingAnchor constant:-20.0],
            [details.bottomAnchor constraintEqualToAnchor:card.bottomAnchor constant:-9.0],
        ]];
    ...
    if (self.catalog.count == 0) {
        ...
        empty.text = @"No incrementally imported guests were discovered.";
    ...
    self.statusLabel.text = [NSUserDefaults.standardUserDefaults stringForKey:LCTVResultKey] ?:
        [NSString stringWithFormat:@"Ready. Incremental container update exposes %lu guest(s); guest HOME paths remain stable by bundle ID.", (unsigned long)self.catalog.count];
    self.statusLabel.textColor = UIColor.whiteColor;
    self.statusLabel.font = [UIFont monospacedSystemFontOfSize:16.0 weight:UIFontWeightRegular];
    self.statusLabel.textAlignment = NSTextAlignmentCenter;
    self.statusLabel.numberOfLines = 0;

    [self.view addSubview:title];
    [self.view addSubview:subtitle];
    [self.view addSubview:delta];
    [self.view addSubview:stack];
    [self.view addSubview:self.statusLabel];
    [NSLayoutConstraint activateConstraints:@[
        [title.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor],
        [title.topAnchor constraintEqualToAnchor:self.view.safeAreaLayoutGuide.topAnchor constant:28.0],
        ...
        [delta.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor],
        [delta.topAnchor constraintEqualToAnchor:subtitle.bottomAnchor constant:10.0],
        [stack.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor],
        [stack.topAnchor constraintEqualToAnchor:delta.bottomAnchor constant:18.0],
        [self.statusLabel.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor],
        [self.statusLabel.leadingAnchor constraintGreaterThanOrEqualToAnchor:self.view.leadingAnchor constant:80.0],
        [self.statusLabel.trailingAnchor constraintLessThanOrEqualToAnchor:self.view.trailingAnchor constant:-80.0],
        [self.statusLabel.bottomAnchor constraintEqualToAnchor:self.view.safeAreaLayoutGuide.bottomAnchor constant:-24.0],
    ]];
}
...
- (void)armGuest:(UIButton *)sender {
    ...
    NSString *message = [NSString stringWithFormat:@"MVP7H ARMED %@. Force-close LiveContainerTV, then reopen it.", sender.currentTitle ?: slotName];
    [defaults setObject:message forKey:LCTVResultKey];
    [defaults synchronize];
...
@interface LCTVMVP7HAppDelegate : UIResponder <UIApplicationDelegate>
@property(nonatomic,strong) UIWindow *window;
@end
@implementation LCTVMVP7HAppDelegate
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    (void)application; (void)launchOptions;
    self.window = [[UIWindow alloc] initWithFrame:UIScreen.mainScreen.bounds];
    self.window.rootViewController = [[LCTVMVP7HViewController alloc] init];
...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        return UIApplicationMain(argc, argv, nil, NSStringFromClass(LCTVMVP7HAppDelegate.class));
    }
}
